...
class KonachanInner:
    def __init__(self, page_capacity):
        ...
        self.page_num = 1
        self.base_url = 'https://yande.re/post'
        self.headers = {
    ...
    def new_folder(self, artist_folder_name):
        ...
        新建文件夹
        '''
        folder_path = './public/yandere/{}'.format(artist_folder_name)
    ...
    def parse_detail_page(self, detail_page_url_list, deal_func):
        ...
        for detail_page_url in tqdm(detail_page_url_list):
            ...
            try:
                ...
                artist_name = 'tagme'
                original_file_unchanged_url = html.xpath("//a[@class='original-file-unchanged']/@href")
                original_file_changed = html.xpath("//a[@class='original-file-changed']/@href")
                if original_file_unchanged_url:
                    self.executor.submit(self.downloadPic, original_file_unchanged_url[0], artist_name)
                else:
                    self.executor.submit(self.downloadPic, original_file_changed[0], artist_name)
                continue
            if deal_func(artist_name) is not None:
                ...
                while next_url is not None:
                    ...
                    for img_url in img_url_list:
                        self.executor.submit(self.downloadPic, img_url, artist_name)
